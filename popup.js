...
async function scanNow() {
  ...
  try {
    ...
    // Check if we are on Gmail (tab/url can be undefined for detached windows)
    if (!tab || !tab.url || !tab.url.includes("mail.google.com")) {
      container.innerHTML =
        '<div class="empty-state">Please open Gmail to use this tool.</div>';
      return;
    }
    ...
    chrome.tabs.sendMessage(tab.id, { action: "scan_inbox" }, (response) => {
      if (chrome.runtime.lastError) {
        container.innerHTML =
          '<div class="empty-state">Please refresh your Gmail tab first.</div>';
        return;
      }

      if (!response || response.length === 0) {
        container.innerHTML =
          '<div class="empty-state">No emails found in this view.</div>';
        return;
      }
    ...
    container.innerHTML =
      '<div class="empty-state">Error connecting to browser.</div>';
  }
}
